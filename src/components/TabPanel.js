  ...
  render() {
    ...
    switch (status) {
      ...
      case 'loading':
        ...
        icons.push(
          <div key="downloading" className="downloading">
            <canvas id="downloading" width="132" height="132" />
          </div>
        );
        break;
      case 'file':
            ...
            <Tabs defaultActiveKey="1" size="small" onChange={this.handleChange} tabBarExtraContent={operations}>
              <Tab tab={<span>返回列表值</span>} key="1">
                <div className="column-settings">
                  ...
                  <div className="label_div">
                    <CheckboxGroup
                      options={columns.map(col => ({ label: col.title, value: col.key, disabled: col.disabled }))}
                      value={checkedList.map(col => col.key)}
                      onChange={onChange}
                    />
                  </div>
                </div>
              ...
              {pageType == 'doctorCashList' ? (
                ...
                <Tab tab={<span>筛选</span>} key="2">
                  <div className="search-settings">
                    <div className="label_div label_div2">{search}</div>
              ...
              {pageType == 'morders' ? (
                <Tab tab={<span>发送邮件</span>} key="3">
                  <div className="search-settings">{sendEmail}</div>
                </Tab>
              ) : (
                ''
              )}
              {pageType == 'familyPackage' ? (
                <Tab tab={<span>生成套餐</span>} key="3">
                  <div className="search-settings">
                    <div className="label_div label_div2">{setMeal}</div>
                  </div>
                </Tab>
              ) : (
                ''
