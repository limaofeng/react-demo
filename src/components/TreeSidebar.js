    ...
    render() {
      ...
      return (
        <li className="fan-menu-item">
          <div className={classnames('menu-item-title', { 'menu-item-selected': isSelected(id) })} onClick={this.handleClick} style={{ paddingLeft: 15 * layer }} role="none">
            <div className="item-title"><a>{title}</a></div>
            <i ref={icon => { this.openIcon = icon; }} className={classnames({ rotates: !open })} onClick={this.handleOpenClick} role="none" />
          </div>
          <ul ref={ul => { this.menuItemList = ul; }} className={classnames('fan-menu-item-group-list', { 'fan-menu-hidden': !open })}>
            {children.map(o => React.cloneElement(o, { onSelect, onOpenChange, id: o.key, selected: isSelected(o.key), open: isOpen(o.key), isSelected, isOpen }))}
          </ul>
        </li>
      );
    }
...
export class TreeItem extends PureComponent {
    ...
    render() {
      ...
      return (
        <li className="fan-menu-item">
          <div className={classnames('menu-item-title', { 'menu-item-selected': selected })} onClick={this.handleClick} style={{ paddingLeft: 15 * layer }} role="none">
            <div className="item-title"><a>{title}</a></div>
          </div>
        </li>
      );
