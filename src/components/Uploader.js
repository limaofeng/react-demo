...
class Uploader extends Component {
    ...
    setValue(value) {
      ...
      this.state = {
        ...
        previewImage: '',
        fileList: isarray ? value.map(item => ({
          // uid为0删除时会全删
          uid: item,
          status: 'done',
    ...
    render() {
      ...
      if (showUploadList) {
        if (multiple || !fileList.length) {
          children.push(<div key="multiple" className="upload-action"><Icon type="plus" />
            <div className="ant-upload-text">点击上传</div>
          </div>);
        }
      } else if (mode === 'avatar') {
        const imageUrl = fileList.length ? fileList[0].url : null;
        children.push(imageUrl ? <div key="single">
        ...
        </div> : <div key="single" className="upload-action">
          <div className="ant-upload-text">点击上传</div>
        </div>);
      }
      return (
