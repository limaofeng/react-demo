...
export default class Area extends Component {
  ...
  changeProvince = e => {
    ...
    // 获取城市
    getAreas({ per_page: 200, 'EQS_parent.id': province })
      .then(d => {
  ...
  changeCity = e => {
    ...
    // 获取城市
    getAreas({ per_page: 200, 'EQS_parent.id': city })
      .then(d => {
  ...
  changeArea = e => {
    ...
    this.props.changeValue && typeof this.props.changeValue === 'function'
      ? this.props.changeValue(areas.find(a => a.id == area))
      : '';
    this.setState({ area });
  };
              ...
              <span className="input-icon icon-right">
                <select style={{ width: '100%' }} onChange={this.changeProvince} value={province}>
                  ...
                  {provinces.map((item, i) => (
                    <option key={`province_${i}`} value={item.id}>
                      {item.name}
                    </option>
                  ))}
                </select>
            ...
            <div className="form-group">
              ...
              <span className="input-icon icon-right">
                <select style={{ width: '100%' }} onChange={this.changeCity} value={city}>
                  ...
                  {citys.map((item, i) => (
                    <option key={`city_${i}`} value={item.id}>
                      {item.name}
                    </option>
                  ))}
                </select>
            ...
            <div className="form-group">
              ...
              <span className="input-icon icon-right">
                <select style={{ width: '100%' }} value={area} onChange={this.changeArea}>
                  <option value="">请选择</option>
                  {areas.map((item, i) => (
                    <option key={`area_${i}`} value={item.id}>
                      {item.name}
                    </option>
                  ))}
                </select>
