...
class MenuTreeAction extends PureComponent {
    ...
    showSetModal = ({ parent, createTime, creator, layer, modifier, modifyTime, uuid, __typename, sort, path, ...category }) => {
      this.setState({
        visible: true,
        isnew: false,
    ...
    handleDeleteCategory = () => {
      ...
      confirm({
        ...
        onOk: () => {
          this.remove(getSelectedKey()).then(() => {
    ...
    render() {
      ...
      return (<div className="action-icons">
        ...
        {visible && (isnew ? <CategorySubmitForm
          category={category}
...
class Sider extends PureComponent {
    ...
    componentWillReceiveProps({ categorys: nextCategorys }) {
      ...
      if (!isEqual(prevCategorys, nextCategorys)) {
        this.setState({ categoryTree: Format.tree(nextCategorys, {
          rootKey: item => !item.parent,
          sort: (a, b) => (a.sort > b.sort ? 1 : -1),
          idKey: 'id',
          pidKey: item => item.parent && item.parent.id,
    ...
    crateMenu = (newDate, layer = 1) => {
      const newDoms = newDate.map(item => {
        if (item.children) {
          return (<TreeItemGroup key={item.id} title={item.name} layer={layer}>
