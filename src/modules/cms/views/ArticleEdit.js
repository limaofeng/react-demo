...
class ArticleDelete extends Component {
  ...
  handleDelete = e => {
    ...
    confirm({
      ...
      content: (
        <div>
          你要删除的文章为 {title} <br /> 系统不提供备份与撤销功能，文章将永久删除文章。确认继续吗？
        </div>
      ),
      onOk() {
        remove();
  ...
  render() {
    return (
      <a onClick={this.handleDelete} role="none">
        删除文章
      </a>
    );
  }
...
class ArticleActions extends PureComponent {
  ...
  render() {
    ...
    if (draft) {
      menus.push(
        <li key="draft">
          <a onClick={this.handleSwitch('draft', 'blue', '保存草稿')} role="none">
            保存草稿
          </a>
        </li>
      );
      menus.push(
        <li key="publish">
          <a onClick={this.handleSwitch('publish', 'danger', '立即发布')} role="none">
            立即发布
          </a>
        </li>
      );
      ...
      menus.push(
        <li key="update">
          <a onClick={this.handleSwitch('update', 'blue', '更新文章')} role="none">
            更新文章
          </a>
        </li>
      );
      menus.push(
        <li key="unpublish">
          <a onClick={this.handleSwitch('unpublish', 'danger', '取消发布')} role="none">
            取消发布
          </a>
        </li>
      );
    ...
    menus.push(
      <li key="move">
        <a onClick={this.handleMove} role="none">
          移动栏目
        </a>
      </li>
    );
    ...
    return (
      <div className="btn-group">
        <a
          ...
          role="none"
        >
          {submitting ? <i className="fa fa-circle-o-notch fa-spin" /> : title}
        </a>
        <a className={`btn btn-${style} dropdown-toggle`} data-toggle="dropdown">
          <i className="fa fa-angle-down" />
        </a>
        <ul className={`dropdown-menu dropdown-${style}`}>{menus}</ul>
      </div>
    );
  }
}
...
class ArticleForm extends PureComponent {
  ...
  render() {
    ...
    return (
      <div className={classnames({ 'settings-menu-expanded': openSettings })}>
