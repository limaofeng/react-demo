    ...
    render() {
      ...
      return (<div className="banner-item">
        ...
        <div className="description">
          <Input placeholder="标题" defaultValue={data.title} onChange={e => this.handleChange('title', e.target.value)} />
          <Input placeholder="描述" defaultValue={data.summary} onChange={e => this.handleChange('summary', e.target.value)} />
...
class BannerItems extends Component {
    ...
    render() {
      ...
      const inputs = items.map((item, index) => (
        <div key={item.id}>
          <BannerItem index={index} value={item} onRemove={() => this.remove(index)} onMoveUp={() => this.move(index, 'up')} onMoveDown={() => this.move(index, 'down')} onChange={data => this.handleChange(index, data)} />
        </div>
      ));
      return (
